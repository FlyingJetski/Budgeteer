...
import com.flyingjetski.budgeteer.ui.add.ViewBudgetFragmentDirections
import com.flyingjetski.budgeteer.ui.add.ViewSavingFragmentDirections
import com.flyingjetski.budgeteer.ui.add.ViewWalletFragmentDirections
    ...
    private fun setupUI() {
        ...
        binding.walletListView.setOnItemClickListener{adapterView, view, position, id ->
            Navigation.findNavController(view).navigate(
                SourcesFragmentDirections
                    .actionSourcesFragmentToEditWalletFragment(
                        (binding.walletListView.adapter.getItem(position) as Wallet).id.toString()
                    )
            )
        }
        ...
        binding.budgetListView.setOnItemClickListener{adapterView, view, position, id ->
            Navigation.findNavController(view).navigate(
                SourcesFragmentDirections
                    .actionSourcesFragmentToEditBudgetFragment(
                        (binding.budgetListView.adapter.getItem(position) as Budget).id.toString()
                    )
            )
        }
        ...
        binding.savingListView.setOnItemClickListener{adapterView, view, position, id ->
            Navigation.findNavController(view).navigate(
                SourcesFragmentDirections
                    .actionSourcesFragmentToEditSavingFragment(
                        (binding.savingListView.adapter.getItem(position) as Saving).id.toString()
                    )
            )
        }
    }
